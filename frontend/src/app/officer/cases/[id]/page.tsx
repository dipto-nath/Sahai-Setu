...
export default function CaseDetailsPage() {
  ...
  let care = '';
  let action = '';
  if (assessment?.explanation_json) {
    try {
      const expl = JSON.parse(assessment.explanation_json);
            ...
            <Card className="border-l-4 border-l-blue-600 bg-blue-50/30">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-blue-700/70">
                  Real-time recommendations generated by Contextual AI
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {summary && (
                  <div>
                    <h4 className="text-xs font-bold text-blue-800 uppercase tracking-wider mb-1">Situation Summary</h4>
                    <p className="text-blue-900 text-sm">{summary}</p>
                  </div>
                )}
                {care && (
                  <div>
                    <h4 className="text-xs font-bold text-teal-800 uppercase tracking-wider mb-1">Psychological Care</h4>
                    <p className="text-teal-900 text-sm bg-teal-100/50 p-2 rounded border border-teal-200">{care}</p>
                  </div>
                )}
                {action && (
                  <div>
                    <h4 className="text-xs font-bold text-orange-800 uppercase tracking-wider mb-1">Recommended Action</h4>
                    <p className="text-orange-900 text-sm bg-orange-100/50 p-2 rounded border border-orange-200">{action}</p>
                  </div>
                )}
              </CardContent>
            </Card>
